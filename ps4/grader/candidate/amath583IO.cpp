...
std::vector<std::string> split(const std::string& s, const char delimiter) // TAKEN FROM: https://www.fluentcpp.com/2017/04/21/how-to-split-a-string-in-c/
{
...
void streamCOOMatrix(const COOMatrix &x)
{
    streamCOOMatrix(x, std::cout);
}

void streamCOOMatrix(const COOMatrix &A, std::ostream &outputFile)
{
