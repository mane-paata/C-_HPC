...
void hoistedTiledMultiply2x2(const Matrix& A, const Matrix&B, Matrix&C) {
  for (int i = 0; i < A.numRows(); i += 2) {
    for (int j = 0; j < B.numCols(); j += 2) {
      float t00 = C(i,j);       float t01 = C(i,j+1);
      float t10 = C(i+1,j);     float t11 = C(i+1,j+1);
  ...
  for (int ii = 0; ii < A.numRows(); ii += blocksize) {
	...
	for (int i = ii; i < ii+blocksize; i += 4) {
	  for (int j = jj, jb = 0; j < jj+blocksize; j += 4, jb += 4) {
	    ...
	    float t00 = C(i,  j);       float t01 = C(i,  j+1);
	    float t10 = C(i+1,j);       float t11 = C(i+1,j+1);
	    float t20 = C(i+2,j);       float t21 = C(i+2,j+1);
	    float t30 = C(i+3,j);       float t31 = C(i+3,j+1);
	    float t02 = C(i,  j+2);     float t03 = C(i,  j+3);
	    float t12 = C(i+1,j+2);     float t13 = C(i+1,j+3);
	    float t22 = C(i+2,j+2);     float t23 = C(i+2,j+3);
	    float t32 = C(i+3,j+2);     float t33 = C(i+3,j+3);
...
float oneNorm(const Matrix& A) {
  std::vector<float> v(A.numCols(), 0.0);
...
float infinityNorm(const Matrix& A) {
  std::vector<float> v(A.numRows(), 0.0);
...
void randomizeMatrix(Matrix &A) {
  ...
  std::uniform_real_distribution<float> distribution(2.0, 32.0);
